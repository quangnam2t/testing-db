...
{if isset($cms) && !isset($cms_category)}
	...
	<div class="rte{if $content_only} content_only{/if}">
		{$cms->content}
        <a class="btn-recruitment" style="display:none">Nộp đơn</a>
	</div>
{elseif isset($cms_category)}
	<div class="block-cms">
		...
		{*<p class="title_block">{l s='List of pages in %s:' sprintf=$cms_category->name}</p>*}
			<ul class="bullet list-group clear-fix">
				{foreach from=$cms_pages item=cmspages}
					<li class="position-recruitment">
                    <a class="list-group-item" href="{$link->getCMSLink($cmspages.id_cms, $cmspages.link_rewrite)|escape:'html':'UTF-8'}">
                    <img src="{$base_dir}img/cms/{strtolower($cmspages.link_rewrite)}.png" />
                    {$cmspages.meta_title|escape:'html':'UTF-8'}</a>
                    </li>
				{/foreach}
			</ul>
		{/if}
