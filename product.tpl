...
<div itemscope itemtype="http://schema.org/Product">
	<div class="primary_block row">
			...
			<div id="image-block" class="clearfix">
				...
				{if $product->on_sale}
					<span class="sale-box no-print">
						<span class="sale-label">-{$product->specificPrice.reduction*100}%</span>
					</span>
		...
		<!-- end left infos-->
		<!-- center infos -->
  {$prs_size = $product->getAttributesGroups($cookie->id_lang)}
    {foreach $prs_size as $prs_arr}
    {if isset($prs_arr.id_attribute_group) && (int)$prs_arr.id_attribute_group==3}
            {$clr_prod = $prs_arr.attribute_name}
    {/if}
    {/foreach}  
		<div class="pb-center-column col-xs-12 col-sm-4">
			...
			{if $product->description_short || $packItems|@count >= 0}
                ...
                <div id="short_description_content" class="rte align_justify" itemprop="description">
					...
					{/if}
                    {if $lang_id==1}
                        <div class="short-des-att-bef col-xs-4 clearfix">Brand</div>
                        <div class="short-des-att-aft col-xs-8">{$product->manufacturer_name}</div>
                        <div class="short-des-att-bef col-xs-4 clearfix">Category</div>
                        <div class="short-des-att-aft col-xs-8">{$category->name}</div>
                        <div class="short-des-att-bef col-xs-4 clearfix">Size</div>
                        <div class="short-des-att-aft col-xs-8">
                        {foreach $prs_size as $prs_arr}
                        {if isset($prs_arr.id_attribute_group) && (int)$prs_arr.id_attribute_group!=3}
                                {$prs_arr.attribute_name}
                        {/if}
                        {/foreach}
                        </div> 
                        <div class="short-des-att-bef col-xs-4 clearfix">Color</div>
                        <div class="short-des-att-aft col-xs-8">{$clr_prod}
                        </div>
                    {else} 
                        <div class="short-des-att-bef col-xs-4 clearfix">Hãng SX</div>
                        <div class="short-des-att-aft col-xs-8">{$product->manufacturer_name}</div>
                        <div class="short-des-att-bef col-xs-4 clearfix">Loại</div>
                        <div class="short-des-att-aft col-xs-8">{$category->name}</div>
                        <div class="short-des-att-bef col-xs-4 clearfix">Kích thước</div>
                        <div class="short-des-att-aft col-xs-8">
                        {foreach $prs_size as $prs_arr}
                        {if isset($prs_arr.id_attribute_group) && (int)$prs_arr.id_attribute_group!=3}
                                {$prs_arr.attribute_name}
                        ...
                        <div class="short-des-att-bef col-xs-4 clearfix">Màu sắc</div>
                        <div class="short-des-att-aft col-xs-8">
                        {foreach $prs_size as $prs_arr}
                        {if isset($prs_arr.id_attribute_group) && (int)$prs_arr.id_attribute_group==3}
                                {$prs_arr.attribute_name}
		...
		<div class="pb-right-column col-xs-12 col-sm-4 col-md-3">
			...
			<form id="buy_block"{if $PS_CATALOG_MODE && !isset($groups) && $product->quantity > 0} class="hidden"{/if} action="{$link->getPageLink('cart')|escape:'html':'UTF-8'}" method="post">
				...
				<div class="box-info-product">
							...
							<div id="attributes">
								<div class="clearfix"></div> 
    								{foreach from=$groups key=id_attribute_group item=group}
    									{if ($group.attributes|@count && $id_attribute_group!='4' && $id_attribute_group!='5' && $id_attribute_group!='6')}  
        										<fieldset class="attribute_fieldset">
        											<label class="attribute_label" {if $group.group_type != 'color' && $group.group_type != 'radio'}for="group_{$id_attribute_group|intval}"{/if}>{$group.name|escape:'html':'UTF-8'}&nbsp;</label>
        											{assign var="groupName" value="group_$id_attribute_group"}
        											<div class="attribute_list">
        												{if ($group.group_type == 'select')}
                                                              {if ($id_attribute_group=='1' || $id_attribute_group=='31')}
                                                                ...
                                                                {else}
                                                            <div id="size_custom">                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                
                                                                <input type="radio" checked="checked" name="rdb_size_2" id="rdb_size_usa_2" value="us" /> US
                                                                <input type="radio" name="rdb_size_2" id="rdb_size_uk_2" value="uk" /> UK
                                                                <input type="radio" name="rdb_size_2" id="rdb_size_europe_2" value="eu" /> EU
                                                                <input type="radio" name="rdb_size_2" id="rdb_size_cm_2" value="cm" /> CM*
                                                            </div>
                                                            <select name="{$groupName}" id="group_{$id_attribute_group|intval}" class="form-control attribute_select no-print">
        													       	{foreach from=$group.attributes key=id_attribute item=group_attribute}
        															<option value="{$id_attribute|intval}"{if (isset($smarty.get.$groupName) && $smarty.get.$groupName|intval == $id_attribute) || $group.default == $id_attribute} selected="selected"{/if} title="{$group_attribute|escape:'html':'UTF-8'}">{$group_attribute|escape:'html':'UTF-8'}</option>
                    ...
                    {foreach $prs_size as $size_at}
                    {if $size_at['id_attribute_group']=='9' && $size_at['quantity']!='0'}
                    <div style="display:none" class="pro_attr_name">{$size_at['attribute_name']}</div>
                    <div style="display:none" class="pro_attr_id">{$size_at['id_attribute']}</div>
                    {/if}
                    {if $size_at['id_attribute_group']=='4' || $size_at['id_attribute_group']=='5' || $size_at['id_attribute_group']=='6'}
                    <div style="display:none" class="chk_attr_id">{$size_at['id_attribute']}</div>
                    {/if}
                    {*if $size_at['id_attribute']=='39' || $size_at['id_attribute'] == '40' || $size_at['id_attribute'] == '41'*}
                    ...
                    <div class="simple-callback-form">
                    {if $lang_id==2}
                    <div class="icon-phone">
                    </div>
                    <div class="simple-callback-title">Hỗ trợ đặt hàng</div>
                    <div class="simple-callback-number"> 
                        <a href="tel:0839240549">(08) 3924 0549</a> 
                    </div>
                    {else}
                    <div class="icon-phone">
                    </div>
                    <div class="simple-callback-title">Order Support</div>
                    <div class="simple-callback-number"> 
                        <a href="tel:0839240549">(08) 3924 0549</a> 
                    </div>
                     {/if}
                     {*<input type="text" id="txt_guest_fnumber" name="txt_guest_fnumber" placeholder="Insert Phone Number"/>
                     <input type="text" id="txt_guest_info" name="txt_guest_info" placeholder="Insert FullName"/>
                     <input type="submit" value="Send" id="btn_insert_guest_info" name="btn_insert_guest_info"/>
                    <script>
                     $("#btn_insert_guest_info").submit(function(){
...
{if (isset($quantity_discounts) && count($quantity_discounts) > 0)}
		...
		{/if}
                <div>
                    <div class="fb-comments" data-href="{$link_detail}" data-width="1180" data-numposts="5"></div>
                </div>
		<!--HOOK_PRODUCT_TAB -->
		<section class="page-product-box">
			{$HOOK_PRODUCT_TAB}
									...
									<li class="item product-box ajax_block_product{if $smarty.foreach.accessories_list.first} first_item{elseif $smarty.foreach.accessories_list.last} last_item{else} item{/if} product_accessories_description">
										...
										<div class="clearfix" style="margin-top:5px">
											{if !$PS_CATALOG_MODE && ($accessory.allow_oosp || $accessory.quantity > 0)}
												<div class="no-print">
													<a class="exclusive button ajax_add_to_`cart_button" href="{$link->getPageLink('cart', true, NULL, "qty=1&amp;id_product={$accessory.id_product|intval}&amp;token={$static_token}&amp;add")|escape:'html':'UTF-8'}" data-id-product="{$accessory.id_product|intval}" title="{l s='Add to cart'}">
